...
BASE_DIR = "/home/awx/sensor_reboot_panel"

DB_PATH = os.path.join(BASE_DIR, "sensor_panel.db")



# ANSIBLE DIRECTORY (UPDATED)
...
@app.route("/reboot/<int:sensor_id>", methods=["POST"])

@login_required

def reboot(sensor_id):

    db = get_db()

    sensor = db.execute("SELECT * FROM sensors WHERE id=?", (sensor_id,)).fetchone()
    ...
    cmd = [
        ...
        "--limit", sensor["ansible_host_name"],
...
def manage_users():
    ...
    if request.method == "POST":
        ...
        if delete_id:
            ...
            db.execute("DELETE FROM users WHERE id=? AND role!='superadmin'", (delete_id,))
